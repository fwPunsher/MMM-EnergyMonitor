...
Module.register("MMM-EnergyMonitor", {
	...
	addIcons: function(wrapper) {
		...
		batteryIcon.className = this.batteryIconFromSoc(batteryIcon);


		const socLabel = document.createElement("div");
		socLabel.id = "soc-label";
		socLabel.classList.add("label");
		socLabel.innerHTML = `${this.currentData.soc}%<br>`;
		batteryIcon.appendChild(socLabel);

		battery.appendChild(batteryIcon);
		wrapper.appendChild(battery);
	},

	batteryIconFromSoc: function (batteryIcon) {
		if (this.currentData.soc < 20) {
			return "fas fa-battery-empty"
		} else if (this.currentData.soc >= 20 && this.currentData.soc < 45) {
			return "fas fa-battery-quarter"
		} else if (this.currentData.soc >= 45 && this.currentData.soc < 70) {
			return "fas fa-battery-half"
		} else if (this.currentData.soc >= 70 && this.currentData.soc < 95) {
			return "fas fa-battery-three-quarters"
		} else if (this.currentData.soc >= 95) {
			return "fas fa-battery-full"
		} else {
			return this.config.iconCssClasses.energyStorage
		}
	},
	...
	notificationReceived(notification, payload, sender) {
		...
		// Unit: Percent
		// Unit: Watt | cannot be negative
		if (notification === "MMM-EnergyMonitor_SOC_UPDATE") {
			Log.log("Updating SOC")
			if(!this.validateNumberPayload(notification, payload, sender))
				return;

			this.currentData.soc = payload < 0 ? 0 : payload ;
			this.resetCounter.soc = 0;
		}
	},
});
